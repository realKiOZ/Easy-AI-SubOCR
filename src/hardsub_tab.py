...
def create_hardsub_tab(parent, gui_instance):
    ...
    ytdlp_frame = ttk.LabelFrame(hardsub_frame, text="Download from URL", padding=10)
    ytdlp_frame.pack(fill=tk.X, expand=False, pady=(0, 10))
    ...
    btn_download_video = ttk.Button(
        ytdlp_frame,
        text="Download",
        command=gui_instance.start_video_download_thread
    )
    ...
    btn_detect_hardsub = ttk.Button(
        hardsub_settings_frame,
        text="2. Detect Subtitles",
        command=gui_instance.start_hardsub_detection_thread
    )
    btn_detect_hardsub.grid(row=5, column=0, columnspan=3, sticky="ew", pady=5)
